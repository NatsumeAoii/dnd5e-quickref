...
try {
    ...
    console.warn('Could not copy CHANGELOG.md to public:', e.message);
}
...
// 4. Update package-lock.json root metadata
try {
    if (fs.existsSync(packageLockPath)) {
        const pkgStr = fs.readFileSync(packageJsonPath, 'utf8');
        const lockStr = fs.readFileSync(packageLockPath, 'utf8');
        const pkgData = JSON.parse(pkgStr);
        const lockData = JSON.parse(lockStr);
        let changed = false;

        if (lockData.version !== latestVersion) {
            lockData.version = latestVersion;
            changed = true;
        }
        if (lockData.packages?.['']) {
            if (lockData.packages[''].version !== latestVersion) {
                lockData.packages[''].version = latestVersion;
                changed = true;
            }
            if (pkgData.license && lockData.packages[''].license !== pkgData.license) {
                lockData.packages[''].license = pkgData.license;
                changed = true;
            }
        }

        if (changed) {
            fs.writeFileSync(packageLockPath, JSON.stringify(lockData, null, 2) + '\n');
            console.log('Updated package-lock.json root metadata');
        } else {
            console.log('package-lock.json root metadata is already current');
        }
    }
} catch (e) {
...
try {
    ...
    if (versionRegex.test(configTs)) {
        ...
        console.warn('Could not find APP_VERSION declaration in src/config.ts');
    }
} catch (e) {
...
// 6. Update public/sw.js cache version
try {
    const serviceWorker = fs.readFileSync(serviceWorkerPath, 'utf8');
    const versionRegex = /(CACHE_VERSION\s*=\s*')([^']+)(')/;

    if (versionRegex.test(serviceWorker)) {
        const newServiceWorker = serviceWorker.replace(versionRegex, `$1${latestVersion}$3`);
        if (newServiceWorker !== serviceWorker) {
            fs.writeFileSync(serviceWorkerPath, newServiceWorker);
            console.log(`Updated public/sw.js CACHE_VERSION -> ${latestVersion}`);
        } else {
            console.log(`public/sw.js CACHE_VERSION is already ${latestVersion}`);
        }
    } else {
        console.warn('Could not find CACHE_VERSION declaration in public/sw.js');
    }
} catch (e) {
    console.error('Failed to update public/sw.js:', e.message);
    process.exit(1);
}
